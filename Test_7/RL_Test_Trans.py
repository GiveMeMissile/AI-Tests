# I do NOT deserve to even touch a technology as beautiful as AI, but IM DOING IT ANYWAY!!!
# Im way too dumb for ts -_-
# Make sure to fix the positional encoding bug...
...
class AI(Object):
    ...
    def __init__(self, x, y, width, height, window, color, model, player):
        ...
        self.directional_vectors = torch.zeros((NUM_SAVED_FRAMES, OUTPUT_LENGTH), dtype=torch.float32)
        self.memory = torch.zeros((NUM_SAVED_FRAMES, SEQUENCE_LENGTH), dtype=torch.float32)
        self.decide = True

    def apply_mask(self, tensor):
        # Applies a mask to a tensor to mask out all the padding values.
        # Will be implemented later as I am LAZY and STUPID!!!

        attention_mask = (tensor == 0)

    def add_frame_to_memory(self, frame, memory, sequence_length):

        for i in range(NUM_SAVED_FRAMES):
            if ((memory[NUM_SAVED_FRAMES - (i+1)] == torch.zeros(sequence_length, dtype=torch.float32)).sum().item() == sequence_length):
                memory[NUM_SAVED_FRAMES - (i+1)] = frame
                return
        
        memory = torch.cat((memory[1 : NUM_SAVED_FRAMES], frame), dim=0)
        return memory
    ...
    def ai_move(self, glues, current_time):
        ...
        if self.decide:
            ...
            self.add_frame_to_memory(X, self.memory, SEQUENCE_LENGTH)
            ...
            original_output= self.model(self.memory, self.directional_vectors)
            original_output = original_output.squeeze(0)
            output = torch.sigmoid(original_output).to("cpu")
            output = (output > 0.5)
            print(f"Output: {output}")
            # self.directional_vectors = output
            self.memory = self.memory.to("cpu")
            # print(f"Original Output: {original_output}\nOutput: {output}")

            self.directional_vectors = self.add_frame_to_memory(output, self.directional_vectors, OUTPUT_LENGTH)
            self.directional_vectors = torch.cat((self.directional_vectors[1 : NUM_SAVED_FRAMES], torch.zeros(OUTPUT_LENGTH)), dim=0)
            print(f"Directional Vectors: {self.directional_vectors}")

        if (self.directional_vectors[NUM_SAVED_FRAMES-2][0]):
            self.dy -= ACCELERATION
        if (self.directional_vectors[NUM_SAVED_FRAMES-2][1]):
            self.dy += ACCELERATION
        if (self.directional_vectors[NUM_SAVED_FRAMES-2][2]):
            self.dx -= ACCELERATION
        if (self.directional_vectors[NUM_SAVED_FRAMES-2][3]):
            self.dx += ACCELERATION
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        try: 
            return self.dropout(x + self.pos_encoding[:x.size(0), :]) # x and pos_encoding x must have the same shape.
        except Exception as e:
            print(f"Shape of x: {x.shape}\nShape of pos_encoding: {self.pos_encoding.shape}")
            raise e


class Transformer(nn.Module):
    # We will be using an Transformer model in this experiment. This TRANSFORMER will control the AI objects.
    ...
    def __init__(self, num_layers, hidden_size, output_size):
        ...
        self.transformer = nn.Transformer(
            ...
            dropout=0 # No dropout as the model is meant to always be learning and not to be used in production. As this is a simple experiment after all.
            )
        self.positional_encoding = PositionalEncoding(hidden_size, dropout=0, max_length=NUM_SAVED_FRAMES)
    ...
    def forward(self, src, tgt):
        # Comment galore:
        # src: sequence = [aix, aiy, dx, dy, player_x, player_y, glue_x1, glue_y1, glue_x2, glue_y2, ...]
        # where dx and dy are the player's velocity, player_x and player_y are the player's position,
        # src = [NUM_SAVED_FRAMES, sequence length]. Where there will be NUM_SAVED_FRAMES amount of sequances in the tensor.
        # The sequences include the current frame plus the last NUM_SAVED_FRAMES-1 frames.
        # The tgt will be the sequence of the AI's actions, which will be used to train the model.
        # The transformer model will predict which action will be nect in the tgt sequence using info from the src sequence.
        # tgt = [NUM_SAVED_FRAMES, OUTPUT_LENGTH]. Where the final output of the tensor will be masked and must be predicted by the model.
        ...
        # Output tensor: [up, down, left, right]
        # where up, down, left, and right are the AI's actions to move. Each variable will be between 0 and 1 (false and true). 
        return True
...
def main():
    ...
    for _ in range(GLUES):
        ...
        glues.append(glue)
    model = Transformer(NUM_LAYERS, HIDDEN_SIZE, OUTPUT_SIZE).to(device)
