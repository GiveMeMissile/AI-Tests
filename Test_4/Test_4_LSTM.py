...
# This is a simple LSTM model used for text classification. Yay
class ClassificationLSTM(nn.Module):
    def __init__(self, input_features, output_features, hidden_features, lstm_layers, num_tokens):
        super(ClassificationLSTM, self).__init__()
        self.hidden_features = hidden_features
        self.lstm_layers = lstm_layers

        self.embed = nn.Embedding(num_tokens, input_features)
        self.input_LSTM_layer = nn.LSTM(input_size=input_features,
    ...
    def forward(self, x, h0, c0):
        if h0 is None:
            h0 = torch.zeros(self.lstm_layers, x.size(0), self.hidden_features)
        if c0 is None:
            c0 = torch.zeros(self.lstm_layers, x.size(0), self.hidden_features)

        x = self.embed(x)
        x, (hn, cn) = self.input_LSTM_layer(x, (h0, c0))
...
def train(dataloader, model, loss_fn, optimizer, h0, c0):
    start = time.time()
    train_loss = 0
    train_accuracy = 0
    model.train()
    for batch, data in enumerate(dataloader):
        ...
        y_logits, h0, c0 = model.forward(X, h0, c0)
    ...
    return train_loss, train_accuracy, train_time, h0, c0


def test(dataloader, model, loss_fn, h0, c0):
    start = time.time()
    test_loss = 0
    test_accuracy = 0
    model.eval()
    with torch.inference_mode():
        ...
        test_time = end-start
        return test_loss, test_accuracy, test_time, h0, c0
...
def main():
    ...
    model = ClassificationLSTM(input_features=INPUT_FEATURES, output_features=OUTPUT_FEATURES,
    ...
    print("\nStating training process...")

    h0, c0 = None, None

    for epoch in range(epochs):
        train_loss, train_accuracy, train_time, h0, c0 = train(train_dataloader, model, loss_fn, optimizer, h0, c0)
        test_loss, test_accuracy, test_time, h0, c0 = test(test_dataloader, model, loss_fn, h0, c0)
