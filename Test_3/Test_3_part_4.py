...
def simulation_display(goal, ai, target, ai2, ai3):
    pygame.draw.rect(WINDOW, BLACK, BACKGROUND)
    target.display_target()
    goal.display_goal()
    ai.display_ai_object()
    ai2.display_ai_object()
    ai3.display_ai_object()
...
def main():
    ...
    model_2 = AIModel(input_features=INPUT_FEATURES,
                      output_features=OUTPUT_FEATURES,
                      hidden_layers=DEFAULT_HIDDEN_LAYERS,
    ...
    physics_manager_ai_2 = PhysicsManager()
    physics_manager_ai_3 = PhysicsManager()
    ...
    ai_object_2 = AIObject(
        ...
        name="model_2",
        collision_relocate=True,
        physics_manager=physics_manager_ai_2,
        learning=True
    )
    ai_object_3 = AIObject(
        model=model_3,
        dims=AI_OBJECT_DIMS,
        ...
        name="model_3",
        collision_relocate=True,
        physics_manager=physics_manager_ai_3,
        learning=True
    )
    ...
    while running:
        ...
        ai_object_2.predict_and_enact_movement()
        ai_object_2.check_for_collisions(current_time)
        ai_object_3.predict_and_enact_movement()
        ai_object_3.check_for_collisions(current_time)
        simulation_display(goal, ai_object_0, target, ai_object_2, ai_object_3)
    pygame.display.quit()
    ai_object_0.print_results(current_time)
    ai_object_2.print_results(current_time)
    ai_object_3.print_results(current_time)
    save_model(model_0)
    save_model(model_2)
    save_model(model_3)
