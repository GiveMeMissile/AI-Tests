...
def simulation_display(goal, ais, target):
...
def main():
    ...
    physics_manager_ai_1 = PhysicsManager()
    ...
    ai_objects = [ai_object_0, ai_object_1, ai_object_2, ai_object_3, ai_object_4]
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                running = False
        goal.check_for_relocation(current_time)
        target.move()
        for ai_object in ai_objects:
            ai_object.predict_and_enact_movement(),
            ai_object.check_for_collisions(current_time)
        simulation_display(goal, ai_objects, target)
    pygame.display.quit()
    for ai_object in ai_objects:
        ai_object.print_results(current_time)
        save_model(ai_object.model)
